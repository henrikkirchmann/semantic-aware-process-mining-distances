...
from distances.activity_distances.gamallo_fernandez_2023_context_based_representations.src.embeddings_generator.utils import DataFrameFields
from pathlib import Path
...
class EventlogDataset:
    ...
    def read_split(self, split: str, cv_fold: int) -> pd.DataFrame:
        ...
        path_to_eventlog = Path(
            self.directory).resolve() / "crossvalidation" / f"fold{cv_fold}_{split}_{self.filename}.csv"

        df = pd.read_csv(str(path_to_eventlog), index_col=0)
