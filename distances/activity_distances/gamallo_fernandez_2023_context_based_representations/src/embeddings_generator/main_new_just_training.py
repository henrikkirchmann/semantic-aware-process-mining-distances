...
from data.write_control_flow_log_to_csv import transform_control_flow_lists_to_csv
# from data.raw_datasets.write_control_flow_to_xes import transform_control_flow_lists_to_xes
from src.data_processor.main import write_csvs
import pytz
from pm4py.objects.log.obj import EventLog, Trace, Event
from pm4py.util.xes_constants import DEFAULT_NAME_KEY, DEFAULT_TRACEID_KEY, DEFAULT_TIMESTAMP_KEY


def main(control_flow_lists, log_name):
    xes_name, output_file = transform_control_flow_lists_to_xes(control_flow_lists)

    write_csvs(xes_name, output_file)

    csv_filename = xes_name.replace(".xes", ".csv")
...
if __name__ == "__main__":
    ...
    main(control_flow_lists, log_name)  # Now calling main() will behave as if it was called from the command line
