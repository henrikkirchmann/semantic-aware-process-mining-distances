...
class BasicCNN(pl.LightningModule):
    def __init__(self, input_type: str, max_len: int, num_categories: int,
                 emb_size: int, embeddings_dict: dict):
        """
        :param input_type: 'onehot', 'emblayer' or 'embeddings'
        :param max_len: Maximum length of the prefix
        ...
        if self.input_type == 'onehot':
            self.act_output = nn.Linear(in_features=(self.num_categories) * self.hidden_channels,
                                        out_features=self.num_categories+1)
        if self.input_type == 'emblayer':
            ...
            self.act_output = nn.Linear(in_features=(self.emb_size - 1) * self.hidden_channels,
                                        out_features=self.num_categories+1)
        ...
        self.conv = nn.Conv1d(in_channels=self.max_len, out_channels=self.hidden_channels, kernel_size=3, stride=1, padding=1)
...
def run_basicCNN_model(eventlog: EventlogDataset, input_type: str, emb_size: int,
    ...
    model = BasicCNN(input_type, eventlog.max_len_case, eventlog.num_activities,
                     emb_size, embeddings_dict)

    path = "./models/" + eventlog.filename + "/basicCNN/checkpoints/"
    ...
    best_model = BasicCNN.load_from_checkpoint(
        path + "best_model.ckpt",
        input_type=input_type,
        max_len=eventlog.max_len_case,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Execute the training of the Basic CNN")
    ...
    logger.log_console(f'TESTING BasicCNN MODEL...')
    ...
    if args.holdout:
        ...
        if args.input_type == 'onehot':
            accuracy, f1_score = run_basicCNN_model(eventlog, args.input_type, 0, dict())
        elif args.input_type == 'emblayer':
            accuracy, f1_score = run_basicCNN_model(eventlog, args.input_type, args.emb_size, dict())
            ...
            accuracy, f1_score = run_basicCNN_model(eventlog, args.input_type, args.emb_size, embeddings_dict)
        else:
        ...
        logger.log_metrics_holdout(Path(args.dataset).stem, 'BasicCNN',
        ...
        logger.log_metrics_cv(Path(args.dataset).stem, 'BasicCNN',
                              str(emb_type) if emb_type else args.input_type,
                              args.emb_size if args.emb_size else 0,
                              args.win_size if args.win_size else 0,
