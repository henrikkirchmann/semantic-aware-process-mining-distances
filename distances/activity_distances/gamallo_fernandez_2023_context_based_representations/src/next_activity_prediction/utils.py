...
class Config:
    ...
    RESULTS_PATH = os.path.abspath(os.path.join(current_path, '../../results'))
...
def get_embeddings(filename, attr_to_emb, emb_type, emb_size, win_size,
    ...
    if fold is None:
        path = path + '/holdout/' + filename + '/' + attr_to_emb + '_' + str(emb_type)
    else:
        path = path + '/crossvalidation/' + filename + '/' + attr_to_emb + '_' + str(emb_type) + '_fold' + str(fold)
